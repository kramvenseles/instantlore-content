...
local complained = false -- «пепел без маяка» сказано, повторять незачем
...
lore.every(1.0, function()
  ...
  if #found == 0 then
    -- Пепел без маяка — обычное дело на краю поля. Жалуемся один раз: дозор
    -- ходит раз в секунду, и строка в лог на каждый круг забивала бы его целиком.
    if not complained then
      complained = true
      lore.log("пепел есть, маяка в 14 блоках нет — сцена не начата")
    end
    return
  end
  complained = false
