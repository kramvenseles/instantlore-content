...
-- ─────────────────────────────────────────────────────────
-- Где КЛИЕНТ считает игрока и транспорт. Нужен, чтобы отличить «сервер посадил не туда»
-- от «сиденье не доехало до клиента»: серверную позицию печатает сама игра, а вторая
-- половина видна только отсюда.
-- ─────────────────────────────────────────────────────────
lore.on("polygon.where", function()
  local p = lore.player()
  if not p then return end
  lore.log(string.format("КЛИЕНТ: игрок (%.3f %.3f %.3f) верхом=%s",
    p.x, p.y, p.z, tostring(p.riding)))

  for _, e in ipairs(lore.entities(12)) do
    if e.slot then
      lore.log(string.format("КЛИЕНТ: сущность слот %s (%.3f %.3f %.3f) в %.1f",
        tostring(e.slot), e.x, e.y, e.z, e.distance))
    end
  end
end)
